...
impl VanityAddr {
    ...
    pub fn validate_eth_input(string: &str, fast_mode: bool) -> Result<(), BtcVanityError> {
        // 1) If length > 12 and `fast_mode` is true, reject (too long).
        ...
        if string.chars().any(|c| !c.is_ascii_hexdigit()) {
            return Err(BtcVanityError::InputNotBase16);
        }
    ...
    pub fn validate_eth_regex_input(regex_str: &str) -> Result<(), BtcVanityError> {
        // For each character in the pattern:
        for c in regex_str.chars() {
            // 1) If it's a recognized regex meta character, allow it.
            if ALLOWED_REGEX_META.contains(&c) {
            ...
            if c.is_alphanumeric() {
                if !c.is_ascii_hexdigit() {
                    return Err(BtcVanityError::RegexNotBase16);
                }
            } else {
                // 3) Neither a recognized meta char, nor a valid base16 alphanumeric => reject.
                return Err(BtcVanityError::InvalidRegex);
