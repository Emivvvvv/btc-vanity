...
fn derive_ed25519_scalar_from_digest(digest: ptr<function, array<u32, 64>>) -> BigInt {
    var scalar_be: array<u32, 32>;
    for (var i: u32 = 0u; i < 32u; i = i + 1u) {
        let digest_index = 31u - i;
        var scalar_byte = (*digest)[digest_index];
        if (digest_index == 0u) {
            scalar_byte = scalar_byte & 0xf8u;
        }
        if (digest_index == 31u) {
            scalar_byte = (scalar_byte & 0x3fu) | 0x40u;
        }
        scalar_be[i] = scalar_byte;
    }
...
fn derive_solana_address(seed_scalar: ptr<function, BigInt>, debug_out: ptr<function, array<u32, 64>>) -> AddressBuf {
    ...
    var ed_scalar = derive_ed25519_scalar_from_digest(&digest_full);
