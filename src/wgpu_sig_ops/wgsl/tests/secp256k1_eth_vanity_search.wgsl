...
fn derive_eth_address(scalar: ptr<function, BigInt>) -> AddressBuf {
...
@compute
@workgroup_size(256)
fn secp256k1_eth_vanity_search(@builtin(global_invocation_id) global_id: vec3<u32>) {
    if (result_words[0] != RESULT_SENTINEL) {
        return;
    }
    ...
    for (var lane: u32 = 0u; lane < candidates_per_invocation; lane = lane + 1u) {
        ...
        var address = derive_eth_address(&scalar);
